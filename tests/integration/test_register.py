...
`register_new_installs` setting baked into tests/integration/docker/settings.yaml.
Register.run() requires root (os.getuid() != 0), so these invoke the real CLI entry
point via `sudo` rather than calling Register() in-process.
"""
